...
test("loads tui config with the same precedence order as server config paths", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await fs.mkdir(path.join(dir, ".elastic-ramen"), { recursive: true })
      await Bun.write(
        path.join(dir, ".elastic-ramen", "tui.json"),
...
test("migrates tui-specific keys from opencode.json when tui.json does not exist", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      const server = JSON.parse(await Filesystem.readText(path.join(tmp.path, "elastic_ramen.json")))
      expect(server.theme).toBeUndefined()
      expect(server.keybinds).toBeUndefined()
      expect(server.tui).toBeUndefined()
      expect(await Filesystem.exists(path.join(tmp.path, "elastic_ramen.json.tui-migration.bak"))).toBe(true)
...
test("skips migration when opencode.jsonc is syntactically invalid", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      expect(await Filesystem.exists(path.join(tmp.path, "elastic_ramen.jsonc.tui-migration.bak"))).toBe(false)
      const source = await Filesystem.readText(path.join(tmp.path, "elastic_ramen.jsonc"))
...
test("skips migration when tui.json already exists", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(path.join(dir, "elastic_ramen.json"), JSON.stringify({ theme: "legacy" }, null, 2))
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      expect(await Filesystem.exists(path.join(tmp.path, "elastic_ramen.json.tui-migration.bak"))).toBe(false)
    },
  })
})
...
test("continues loading tui config when legacy source cannot be stripped", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(path.join(dir, "elastic_ramen.json"), JSON.stringify({ theme: "readonly-theme" }, null, 2))
    },
  })

  const source = path.join(tmp.path, "elastic_ramen.json")
...
test("migration backup preserves JSONC comments", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      await TuiConfig.get()
      const backup = await Filesystem.readText(path.join(tmp.path, "elastic_ramen.jsonc.tui-migration.bak"))
...
test("migrates legacy tui keys across multiple opencode.json levels", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Bun.write(path.join(dir, "elastic_ramen.json"), JSON.stringify({ theme: "root-theme" }, null, 2))
      await Bun.write(path.join(nested, "elastic_ramen.json"), JSON.stringify({ theme: "nested-theme" }, null, 2))
    },
  })
...
test("does not derive tui path from OPENCODE_CONFIG", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Bun.write(path.join(customDir, "elastic_ramen.json"), JSON.stringify({ model: "test/model" }))
      await Bun.write(path.join(customDir, "tui.json"), JSON.stringify({ theme: "should-not-load" }))
      process.env.OPENCODE_CONFIG = path.join(customDir, "elastic_ramen.json")
    },
  })
...
test("loads .elastic-ramen/tui.json", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await fs.mkdir(path.join(dir, ".elastic-ramen"), { recursive: true })
      await Bun.write(path.join(dir, ".elastic-ramen", "tui.json"), JSON.stringify({ diff_style: "stacked" }, null, 2))
    },
  })
