  ...
  async handler(args) {
    await Instance.provide({
      ...
      async fn() {
          ...
          if (prInfoText.trim()) {
            ...
            if (prInfo && prInfo.body) {
              ...
              if (sessionMatch) {
                ...
                UI.println(`Found elastic-console session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await Process.text(["elastic-console", "import", sessionUrl], {
                  nothrow: true,
                })
        ...
        UI.println()
        UI.println("Starting elastic-console...")
        UI.println()

        // Launch elastic-console TUI with session ID if available
        const { spawn } = await import("child_process")
        const opencodeArgs = sessionId ? ["-s", sessionId] : []
        const opencodeProcess = spawn("elastic-console", opencodeArgs, {
          stdio: "inherit",
          cwd: process.cwd(),
        })
        ...
        await new Promise<void>((resolve, reject) => {
          opencodeProcess.on("exit", (code) => {
            if (code === 0) resolve()
            else reject(new Error(`elastic-console exited with code ${code}`))
          })
