...
// and has been modified by Elastic NV. Changes: updated agent switch from build to investigate on plan_exit, rebranded CLI resume command from elastic-console to ramen
import {
  batch,
  createContext,
  createEffect,
  ...
  createEffect(() => {
    ...
    return exit.message.set(
      [
        ...
        `  ${weak("Continue")}${UI.Style.TEXT_NORMAL_BOLD}elastic-ramen -s ${session()?.id}${UI.Style.TEXT_NORMAL}`,
        ``,
      ].join("\n"),
    )
  })
              ...
              <For each={d.columns}>
                {(col, ci) => {
                  ...
                  return (
                    <text>
                      <span style={{ fg: theme.textMuted }}>│ </span>
                      <span style={{ fg: theme.text }}>{padText(row.label + colSuffix, lw + colSuffix.length)}</span>
                      <span style={{ fg: theme.textMuted }}> │ </span>
                      <span style={{ fg: colors[ci() % colors.length] }}>{bar}</span>
                      <span style={{ fg: theme.textMuted }}> │ </span>
                      <span style={{ fg: theme.text }}>{fmtNum(val)}</span>
                    </text>
                  )
                }}
              </For>
              <Show when={d.columns.length > 1}>
                <text> </text>
              </Show>
            </>
          )}
